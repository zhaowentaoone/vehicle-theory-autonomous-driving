...
traj = zeros(3,n+1); eys = zeros(1,n+1); traj(:,1)=x;
for k = 1:n
    [Yp, psip] = path_at(x(1));
    ey = x(2) - Yp;
    epsi = mod(x(3)-psip+pi, 2*pi)-pi;
    delta = max(min(-(K * [ey; epsi]), deg2rad(35)), -deg2rad(35));
    x = bicycle_euler(x, v, delta, dt, p.L);
    traj(:,k+1)=x;
    eys(k+1) = x(2) - path_at(x(1));
end
fprintf('RMSE ey=%.3f m\n', rms(eys));
...
subplot(1,2,2); plot(linspace(0,T,n+1), eys, 'k-'); grid on; xlabel('t'); ylabel('ey');
sgtitle('Kinematic LQR lane keeping');
